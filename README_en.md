...
| Workspace | `chattool setup workspace` | Create a collaboration workspace around a core project with `projects/` as the execution container and workspace-level files as the general-use protocol layer; supports `--with-chattool`, `--with-chatblog`, and `--with-memory` to attach ChatTool, ChatBlog, and ChatMemory/ChatArch skills |
