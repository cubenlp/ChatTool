...
`tests/mock-cli-tests/` 收纳 ChatTool 仓库内所有基于 mock 的 CLI 测试，采用文档先行：先写 `.md`，再实现 `.py`。
...
- `tests/mock-cli-tests/` 用于验证 CLI 入口编排、参数流向、输出格式、懒加载和交互分支。
- `tests/cli-tests/` 只保留真实 CLI 链路与真实环境验收。
