...
Run only these tests:
    ...
    pytest tests/code-tests/tools/lark/test_bot_integration.py -v -m lark
...
def assert_ok(response, label: str = ""):
    ...
    if not response.success():
        ...
        if code in (230013, 99991672):
            ...
            hint = "\n  → 用户不在应用可见范围内"
        pytest.fail(f"[{label}] API 调用失败: code={code}  msg={response.msg}{hint}")
...
def require_test_user() -> str:
    ...
    if not TEST_USER_ID:
        pytest.skip(
            "需要在 `.env` 或环境变量中设置 FEISHU_DEFAULT_RECEIVER_ID，才能执行真实飞书消息测试"
        )
    return TEST_USER_ID
...
def test_send_text_to_user(lark_bot):
    ...
    resp = lark_bot.send_text(
        require_test_user(), USER_ID_TYPE, "[test] 你好，这是文本消息 👋"
    )
...
def test_reply_to_message(lark_bot: LarkBot):
    ...
    send_resp = lark_bot.send_text(
        test_user_id, USER_ID_TYPE, "[test] 原始消息（将被回复）"
    )
...
def test_reply_card_to_message(lark_bot: LarkBot):
    """发一条消息，然后用卡片回复它。"""
    send_resp = lark_bot.send_text(
        require_test_user(), USER_ID_TYPE, "[test] 等待卡片回复..."
    )
    ...
    card = {
        "config": {"wide_screen_mode": True},
        "header": {
            "title": {"tag": "plain_text", "content": "[test] 卡片回复"},
            "template": "green",
        },
        "elements": [
            {
...
def test_on_message_dispatch_and_reply(lark_bot: LarkBot):
    ...
    sid = MagicMock()
    sid.open_id = TEST_USER_ID or "feishu_test_user"
    sender = MagicMock()
