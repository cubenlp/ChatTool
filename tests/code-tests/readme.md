...
`tests/code-tests/` 收纳非 CLI 的代码测试，以及从旧 `tests/` 目录迁移过来的历史测试。

- 这里主要放库级行为、工具实现、兼容性和历史回归测试。
- 新的 CLI 文档驱动测试应写到 `tests/cli-tests/` 或 `tests/mock-cli-tests/`。
- 若旧测试仍有参考价值，可继续保留在这里，但不要把它当作新的 CLI 规范来源。

另外，`tests/code-tests/dns/test_cert_*_real.py` 这类真实证书生命周期测试默认不参与日常全量回归；如需显式开启，先设置 `CHATTOOL_RUN_DNS_CERT_REAL=1`。
