...
`tests/cli-tests/` 是 ChatTool 仓库内真实 CLI 测试的设计入口，采用文档先行：先写 `.md`，再实现 `.py`。
...
- ChatTool 仓库长期维护两条 CLI 测试线：`tests/cli-tests/` 负责真实链路，`tests/mock-cli-tests/` 负责 mock 链路。
- `tests/code-tests/` 为代码测试与历史迁移区，不再作为新开发默认 CLI 测试落点。
- 新 CLI 行为必须先补 `.md`，没有对应 `.md` 的 `.py` 不应新增。
- 缺少真实环境/变量的用例先保留 `.md`，待环境满足后再补 `.py`。
- 使用 `mock`、`patch`、`monkeypatch`、fake client / API 的 CLI 测试，必须写到 `tests/mock-cli-tests/`。
- Feishu 的任务实践文档统一按能力落到 `tests/cli-tests/lark/<topic>/`，先描述真实任务，再补自动化实现。
