...
def test_setup_lark_cli_env_ref_uses_feishu_profile_priority(tmp_path: Path):
    ...
    result = _run_python(
        "\n".join(
            [
                "import json",
                "from chattool.setup.lark_cli import _load_feishu_values_from_env_ref, _snapshot_feishu_values",
                "payload = {",
                "    'env_ref': _load_feishu_values_from_env_ref('work'),",
                "    'current': _snapshot_feishu_values(),",
                "}",
    ...
    assert payload["env_ref"]["app_id"] == "from_profile"
    assert payload["env_ref"]["app_secret"] == "profile_secret"
    assert payload["env_ref"]["api_base"] == "https://open.larksuite.com"
    assert payload["current"]["app_id"] == "from_os"
    assert payload["current"]["app_secret"] == "os_secret"
    assert payload["current"]["api_base"] == "https://open.larksuite.com"
