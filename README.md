...
| 环境安装 | `chattool setup codex/claude/opencode/hermes/lark-cli/docker` | 安装或检查常用 Agent CLI / WebUI / Docker 环境，并在确认后执行建议的系统命令；`setup hermes` 可安装 Hermes Agent/WebUI 并复用 OpenAI、Feishu 配置；`setup opencode/codex/claude` 现支持 `--install-only` 纯安装/升级，`setup opencode` 也支持 `--plugin auto-loop` 追加写入 `opencode-auto-loop` |
