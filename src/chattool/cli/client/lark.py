...
@click.option("--card", "-c", "card_file",
              ...
              help="发送卡片消息（JSON 文件路径），此时 TEXT 参数被忽略")
@click.option("--post", "-p", "post_file",
              type=click.Path(exists=True),
              help="发送富文本消息（JSON 文件路径），此时 TEXT 参数被忽略")
def send(receiver, text, id_type, card_file, post_file):
    """
    发送消息给指定用户或群。
    ...
    示例:
      ...
      chattool lark send rexwzh "ignored" --card card.json
...
def listen(verbose):
    ...
    click.secho("🔗 启动 WebSocket 监听... (Ctrl-C 停止)", fg="green")
    try:
        ws.start()
