...
from chattool.docker.elements import TEMPLATES, USAGE, parse_set_values, render_env_example, resolve_template
from chattool.utils.tui import BACK_VALUE, ask_path, ask_select, is_interactive_available


def docker_main(template, output_dir, interactive, set_values, compose_name, env_name, force):
    ...
    if need_prompt and not is_interactive_available():
        if force_interactive:
            ...
            click.echo("Missing required arguments and no TTY is available for interactive prompts.", err=True)
    ...
    env_file.write_text(render_env_example(selected_template, overrides), encoding="utf-8")
