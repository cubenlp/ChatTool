...
description: Handle ChatTool release readiness and post-merge release execution. Use when the task includes version bumps, changelog finalization, tag timing, Publish Package workflow checks, or PyPI verification.
version: 0.1.3
---
...
- You need to verify PyPI state after a real release.
...
- Before merge or tag creation, verify `chatgh pr checks <pr> --repo cubenlp/ChatTool` has reached terminal green status before release decisions; rerun after GitHub finishes in-progress checks.
