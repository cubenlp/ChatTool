...
Use plain `pr-check` only when you explicitly want a one-shot snapshot.
...
- When CI is still running, prefer `pr-check --wait` instead of manually re-running status commands.
