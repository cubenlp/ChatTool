...
4. **ChatTool `.env`**：`chatenv` 管理的类型化 env 文件。
5. **默认值**：代码中定义的默认值。
...
- 这里的 `ChatTool .env` 指 `chatenv` 管理的类型化 env 文件，不是当前工作目录下的 `.env`。
- 当命令显式传入 `-e/--env` 时，会走显式 env 覆盖源，并保持它高于工具配置、系统环境变量与 ChatTool `.env`。
...
- **OpenAI**: `OPENAI_API_KEY`, `OPENAI_API_BASE`, `OPENAI_API_MODEL`, `OPENAI_ACCESS_TOKEN`, `OPENAI_REFRESH_TOKEN`, `OPENAI_OAUTH_TOKEN_URL`, `OPENAI_ACCESS_TOKEN_EXPIRES_AT`, `OPENAI_IMAGE_MODEL` 等；OAuth token、OAuth token endpoint、access token 过期时间与默认 image model 也收纳在同一 OAI/OpenAI 配置类型下，不再拆分独立 Codex 配置类型。
