...
- **OpenAI**: `OPENAI_API_KEY`, `OPENAI_API_BASE`, `OPENAI_API_MODEL`, `OPENAI_ACCESS_TOKEN`, `OPENAI_REFRESH_TOKEN`, `OPENAI_IMAGE_MODEL` 等；OAuth token 与默认 image model 也收纳在同一 OAI/OpenAI 配置类型下，不再拆分独立 Codex 配置类型。
