...
- 将复杂专题能力写成 `skills/feishu/` 子目录里的专题 `.md`，而不是拆 sibling skill 目录。
- 将官方 API、格式约束、字段结构等内容下沉为主题子目录里的 reference 文档，而不是直接塞进主 skill。
- 根目录只保留 `SKILL.md` 作为索引；安装时会整目录拷贝，因此主 skill 包内可以直接放专题子目录、reference 与样例说明。
...
- 根目录只保留 `SKILL.md` 作为单一入口。
- 真正的工作材料下沉到主题子目录里的专题 `.md`。
- reference 资料也随主题子目录一起组织，不再额外拆飞书 sibling skill 目录。
