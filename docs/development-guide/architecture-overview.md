...
- `src/chattool/tools/`：Lark、Image、Network、Browser、Zulip、CC 等仍由 ChatTool parent 维护的核心工具；成熟能力可拆到独立包（例如 DNS -> `ChatDNS`）。
...
这条路线在 [docs/development-guide/task-driven-iteration.md](task-driven-iteration.md) 中已经明确：

1. 先完成任务。
2. 判断能力是通用实现、CLI/MCP/service，还是 agent workflow 知识。
3. 通用能力收敛到 `src/`。
4. Agent workflow 知识沉淀到 workspace/ChatMemory 共享 skills，不再写入 ChatTool 仓库根 `skills/`。
5. 同步文档、测试和导航。
...
- 定义 ChatTool 的具体 env schema，并注册给独立 `chatenv` CLI。
- 保留 OpenAI、DNS、Feishu、GitHub 等业务配置与测试逻辑。
- 避免业务代码直接散落地读取 `os.environ`。

它解决的是“ChatTool 有哪些配置类型”的问题；通用 env/profile CLI 由 `chatenv` 提供。
...
- 从显式 `--source`、`CHATTOOL_SKILLS_DIR` 或 ChatEnv 配置读取外部 skills。
- 把外部 skills 安装到 Codex / Claude / OpenCode 等目标平台。
- 保持 ChatTool 源码仓库与 workspace/ChatMemory 共享 skills 的职责分离。

它解决的是“如何把外部技能源安装到目标 Agent 平台”的问题；ChatTool 仓库不再维护根 `skills/` 资产。
...
- `src/` 中的正式能力实现
- `tests/cli-tests/` 中围绕真实 CLI 行为的 doc-first 测试设计与执行实现
- `tests/mock-cli-tests/` 中围绕 CLI 编排与 mock 场景的 doc-first 测试设计与执行实现
...
- 再按需要接入 `client/`
- 若要服务化，则接入 `serve/`
- 若要暴露给 Agent，则在 workspace/ChatMemory 共享 skills 中沉淀流程知识，而不是写入 ChatTool 仓库根 `skills/`
...
4. 若能力对应稳定 agent workflow，再沉淀到 workspace/ChatMemory 共享 skills。
5. 同步补 `tests/cli-tests/` 或 `tests/mock-cli-tests/`、`docs/`、`mkdocs.yml`；`tests/code-tests/` 不再作为新 CLI 功能默认维护面。
...
`真实任务 -> 核心实现 -> CLI 固化 -> 协议/服务接入 -> workspace/ChatMemory Skill 场景化 -> 文档与测试沉淀`
