...
- 对复用 `config/` 已注册配置项的 setup 命令，默认值来源必须遵循：`显式参数 > 保存的 typed env 配置 > shell 环境变量 > 工具现有配置 > 内置默认值`。
- 对支持 `-e/--env` 的 setup 命令，优先级必须进一步变为：`显式参数 > -e/显式 env > 保存的 typed env 配置 > shell 环境变量 > 工具现有配置 > 内置默认值`。
