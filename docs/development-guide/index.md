...
- 经过这一轮规范化后，`chattool client cert/svg2gif`、独立 `chatgh`、独立 `chatdns`、独立 `chatimg`、独立 `chatzulip`、`chatenv` 高频 profile/key/test 命令、`chattool explore arxiv get`、`chattool tplogin ufw add` 都已经作为参考实现或拆分后入口落到共享机制上；新增 CLI 应直接比照这些命令，而不是再复制旧样板。
