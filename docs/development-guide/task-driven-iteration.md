...
6. **任务后复盘**  
   ...
   - GitHub 流程优先使用独立 `chatgh`，包括 `pr-legacy create`、`pr-legacy edit`、`pr view` 等。
   - 在声称“PR 可合并”或“CI 已通过”之前，必须先同步最新 base 分支，并验证一次真实合并态：`git fetch origin <base>` 后，查看 `chatgh pr view <number>` / `chatgh pr-legacy checks --number <number>` 的 `mergeable`、`mergeable_state`，必要时在最新 base 上做临时 merge/rebase 演练并在该结果上跑最相关测试。
