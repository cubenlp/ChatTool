...
### Added
- 新增根目录 `Dockerfile.playground`，用于直接构建一个最小的 ChatTool Playground 镜像；容器启动后会线性执行 `chattool setup playground -> chattool env set CHATTOOL_SKILLS_DIR -> chattool skill install --all -> chattool setup alias`
