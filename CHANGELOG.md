...
## 2026-06-15
- `chattool image` 新增 `codex` provider：可复用 ChatGPT/Codex OAuth access token（或 `~/.hermes/auth.json` 中的 `openai-codex` 登录）调用 `gpt-image-2`，支持 `list-models`、默认输出到 `./generated/`，并补充对应 typed env、文档与测试。
