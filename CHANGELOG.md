...
- 准备 `7.0.4` 版本但暂不发版：先查询 PyPI 最新 `chattool` 为 `7.0.3`，本次按连续 patch 规则将包版本从未发布的 `7.2.0` 修正为 `7.0.4`；不创建 tag，不触发发布。
- `chattool setup workspace` 生成的归档文件职责调整：根 `ARCHIVE.md` 改为归档操作指南，`archive/README.md` 不再生成，改为 `archive/index.md` 记录已归档内容索引；中英文 workspace 模板、setup 文档和测试同步更新。
- `chattool pypi init -t chatarch` 生成的 PyPI publish workflow 增加 `environment: pypi`，并补充测试防止重新引入仓库级 PyPI token secret。
- GitHub/gh 能力整体迁出 ChatTool，移除顶层 `chattool gh` 入口、默认 `chatgh='chattool gh'` alias、内部 `tools/github` helper、`GitHubClient` 导出、GitHub typed env schema、`github` optional extra、旧 gh mock 测试目录和 GitHub explore 草案；ChatTool 通过 package dependency 引用 `chatgh>=0.2.4`，后续 PR/CI/workflow 操作统一使用独立 `chatgh`。ChatTool 侧文档同步收敛为迁移入口和 token 解析规则，不再维护旧 deprecated 命令细节。
...
- `chattool pypi init -t chatarch` 生成的仓库元数据默认指向 `ChatArch/<package>`，包括 README badge、`pyproject.toml` URLs 与 `mkdocs.yml` 仓库链接。
- `chatarch` 模板生成的 `publish.yml` 改为只由 `v*` tag 或手动触发，并校验 tag 与包版本一致；不再由普通分支 push 触发发布，也不再自动创建 release tag。
...
### Changed
- [2026-04-20] `chatloop` 的项目内事件记录现从 `.opencode/logs/<session-id>.log` 收口为 `.opencode/chatloop.events.log`，并补充对 `session.status=idle`、`session.idle`、`session.error`、`session.deleted`、`permission.asked` 与关键消息事件的详细捕获，便于排查 continuation 没有触发的问题；`/chatloop-status`、help 文案、workspace 模板和 setup 文档同步更新为新的调试路径
